...
begin
  ...
  if b.completion_pin <> p_pin then
    ...
    -- Match FixPados behavior: auto-cancel after 5 wrong PIN attempts.
    if b.pin_attempts + 1 >= 5 then
      ...
      insert into public.audit_logs(user_id, action, target_id, meta)
        values (auth.uid(), 'pin_locked', p_booking, jsonb_build_object('attempts', b.pin_attempts + 1));
      insert into public.audit_logs(user_id, action, target_id, meta)
        values (null, 'booking_cancelled', p_booking, jsonb_build_object('reason','pin_attempts_exceeded','by','system'));
      return jsonb_build_object('ok', false, 'error', 'locked_cancelled');
    end if;
    insert into public.audit_logs(user_id, action, target_id, meta)
...
## 12. MongoDB → Postgres table mapping (current → target)

| Current MongoDB collection (`/app/backend/server.py`) | Target Postgres table             | Notes                                                                |
|-------------------------------------------------------|-----------------------------------|----------------------------------------------------------------------|
| `users`                                               | `users` + `workers`               | Split worker-only fields into `workers`.                             |
| `categories`                                          | `services`                        | Renamed for clarity.                                                 |
| `banners`                                             | `banners`                         | Same.                                                                |
| `addresses`                                           | `addresses`                       | Same.                                                                |
| `bookings`                                            | `bookings`                        | `completion_pin` becomes RLS-hidden; expose via `verify_completion_pin`. |
| `messages` (chat)                                     | `messages`                        | Realtime channel `messages:{booking_id}`.                            |
| `ratings` (currently fields on booking)               | `ratings`                         | Promote to own table for analytics.                                  |
| `notifications`                                       | `notifications`                   | Same.                                                                |
| `otps` / `otp_log`                                    | (none — Supabase Auth handles)    | Drop; rate-limits + expiry done by Supabase.                         |
| `audit_logs`                                          | `audit_logs`                      | Same name; written by triggers & `security definer` functions.       |
| `kyc_docs` (base64 inside `users`)                    | `worker_documents` + Storage `kyc/` | Move binary out of Postgres into private bucket; row stores path.   |

### Data export & one-time migration script (run from a Node script with `mongodb` + `@supabase/supabase-js`)
...
// 1. users (only customers + workers; admin seeded separately)
for await (const u of db.collection('users').find()) {
  await supa.from('users').upsert({
    id: u.id, mobile: u.mobile, role: u.role, name: u.name, photo: u.photo,
  });
  if (u.role === 'worker') {
    await supa.from('workers').upsert({
      user_id: u.id, kyc_status: u.kyc_status ?? 'pending',
      rating: u.rating ?? 0, completed_jobs: u.completed_jobs ?? 0,
      categories: u.categories ?? [], available: u.available ?? true,
    });
    // Move kyc_docs base64 → kyc bucket
    for (const [docType, dataUrl] of Object.entries(u.kyc_docs ?? {})) {
      const [, b64] = (dataUrl as string).split(',');
      const bin = Buffer.from(b64, 'base64');
      const path = `${u.id}/${docType}.bin`;
      await supa.storage.from('kyc').upload(path, bin, { upsert: true });
      await supa.from('worker_documents').insert({
        worker_id: u.id, doc_type: docType, storage_path: path,
        mime: 'image/jpeg', size_bytes: bin.length,
      });
    }
  }
}
...
for await (const c of db.collection('categories').find())
  await supa.from('services').upsert({ id: c.id, name: c.name, icon: c.icon, active: c.active });

// 3. bookings, messages, banners, addresses, audit_logs — straight upserts on `id`
for (const col of ['bookings','messages','banners','addresses','audit_logs','notifications'])
  for await (const d of db.collection(col).find())
    await supa.from(col === 'audit_logs' ? 'audit_logs' : col).upsert({ ...d, _id: undefined });
```

Run with the service-role key in a one-shot maintenance window.

---

## 13. Rollback / dual-run safety net

- Keep the FastAPI service alive but **read-only** for 7 days after cutover.
- Frontend feature-flag (`EXPO_PUBLIC_BACKEND=supabase|fastapi`) — flip back instantly if a regression is found.
- Nightly diff job compares row counts of `bookings`, `users`, `audit_logs` between Mongo and Postgres until both match for 3 consecutive days, then retire the Mongo cluster.

---

## 14. Mapping of current FixPados API → Supabase calls (quick reference)

| FixPados API (FastAPI)                                | Supabase equivalent                                                       |
|-------------------------------------------------------|---------------------------------------------------------------------------|
| `POST /api/auth/send-otp`                             | `supabase.auth.signInWithOtp({ phone })`                                  |
| `POST /api/auth/verify-otp`                           | `supabase.auth.verifyOtp({ phone, token, type:'sms' })`                   |
| `POST /api/auth/admin-login`                          | Email/password sign-in on a seeded admin user                             |
| `GET /api/categories`                                 | `supabase.from('services').select('*')`                                   |
| `POST /api/bookings`                                  | `supabase.from('bookings').insert(...)`                                   |
| `PATCH /api/bookings/{id}/accept` / `/reject`         | `supabase.from('bookings').update({status})`  (RLS allows worker only)    |
| `PATCH /api/bookings/{id}/start`                      | `supabase.from('bookings').update({status:'in_progress'})`                |
| `POST /api/bookings/{id}/verify-pin`                  | `supabase.rpc('verify_completion_pin', {...})`                            |
| `POST /api/worker/upload-kyc`                         | Storage upload to `kyc/<uid>/...` + insert into `worker_documents`        |
| `GET /api/admin/audit-logs`                           | `supabase.from('audit_logs').select('*')` (admin RLS)                     |
| `GET /api/chat/{bid}/messages`                        | `supabase.from('messages').select().eq('booking_id', bid)` + realtime sub |
